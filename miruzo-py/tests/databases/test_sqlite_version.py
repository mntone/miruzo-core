...
@pytest.mark.parametrize('version', ['3', 'x.37.0', '3.37.beta'])
...
def test_verify_sqlite_supports_returning_and_strict_accepts_boundary() -> None:
	verify_sqlite_supports_returning_and_strict('3.37.0')


def test_verify_sqlite_supports_returning_and_strict_rejects_older_version() -> None:
	with pytest.raises(RuntimeError, match='SQLite 3.37.0\\+ is required'):
		verify_sqlite_supports_returning_and_strict('3.36.1')
