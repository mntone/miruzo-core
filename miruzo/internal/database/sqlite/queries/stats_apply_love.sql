...
-- WITH users/*is actually latest; it is workaround*/ AS (
...
-- 	first_loved_at=
-- 		CASE
-- 			WHEN (SELECT occurred_at FROM users) IS NULL THEN NULL
-- 			WHEN stats.first_loved_at IS NULL
-- 			  OR stats.first_loved_at > (SELECT occurred_at FROM users)
-- 			THEN (SELECT occurred_at FROM users)
-- 			ELSE stats.first_loved_at
-- 		END,
-- 	last_loved_at=(SELECT occurred_at FROM users)
